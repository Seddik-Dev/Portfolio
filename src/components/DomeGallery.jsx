...
import angular from "@/assets/logos/angularjs.jpg";
...
export default function DomeGallery({
  ...
  const openItemFromElement = (el) => {
    ...
    temp.style.cssText = `position:absolute;width:${openedImageWidth || `${frameR.width}px`};height:${openedImageHeight || `${frameR.height}px`};visibility:hidden;`;
    document.body.appendChild(temp);
    const tempRect = temp.getBoundingClientRect();
    temp.remove();
    const targetW = Math.max(1, tempRect.width || frameR.width);
    const targetH = Math.max(1, tempRect.height || frameR.height);

    const centeredLeft =
      frameR.left - mainR.left + (frameR.width - targetW) / 2;
    const centeredTop =
      frameR.top - mainR.top + (frameR.height - targetH) / 2;
    ...
    const dx0 = tileR.left - mainR.left - centeredLeft;
    const dy0 = tileR.top - mainR.top - centeredTop;
    ...
    overlay.style.transform = `translate(${dx0}px, ${dy0}px) scale(${validSx0}, ${validSy0})`;
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              ref={frameRef}
              className="viewer-frame w-full h-full flex"
              style={{
